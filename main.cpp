...
inline void vm_forward() { vm.bp = (vm.bp + 1) % DATA_SEGMENT_SIZE; }
inline void vm_backward() {
  vm.bp = (vm.bp + DATA_SEGMENT_SIZE - 1) % DATA_SEGMENT_SIZE;
}
inline void vm_increment() { vm.ds[vm.bp]++; }
inline void vm_decrement() { vm.ds[vm.bp]--; }
inline void vm_input() { vm.ds[vm.bp] = getchar(); }
inline void vm_output() { putchar(vm.ds[vm.bp]); }
inline void vm_while_entry() {
  if (vm.ds[vm.bp]) {
    vm.ss[vm.sp] = vm.ip - 1;
    vm.sp++;
  } else {
    ...
    for (vm.ip++; vm.cs[vm.ip] && c; vm.ip++) {
      if (vm.cs[vm.ip] == '[') c++;
      else if (vm.cs[vm.ip] == ']') c--;
    }
  }
}
inline void vm_while_exit() {
  if (vm.ds[vm.bp]) {
    vm.sp--;
    vm.ip = vm.ss[vm.sp];
  }
